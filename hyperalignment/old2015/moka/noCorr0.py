...
csvout = csv.writer(open('noCorr0-stz.csv', 'w'))
...
for PC in range(2, 1000, 1):
	t = [copy.deepcopy(X) for X in tc]
	s = [copy.deepcopy(X) for X in sc]
	print "\n\nPCA"
	num_pcs = PC
	pca = PCA(num_pcs)
	tData = []
	...
	for holdOut in range(30, 85, 5):
		...
		for part in range(0, len(labels), 7): #I thought I could make this 1... apparently no
			...
			for i in range(1, len(s)):
				#TRAIN.append(tDataTrain[i])		#switch these back in?	
				#LABELS.append(tlabels)						
		...
		print "\n"
		csvout.writerow([PC, holdOut, numpy.mean(score1), numpy.mean(score2), numpy.mean(score3), numpy.mean(score4), numpy.mean(score5), numpy.mean(score6), numpy.mean(score7)])
