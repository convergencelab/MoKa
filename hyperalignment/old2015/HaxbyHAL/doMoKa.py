...
for i in range(1, len(ds_all)):
	#top_voxels_S.append(fast_top_voxels(ds_all[0]['data'], ds_all[i]['data'], len(ds_all[0]['data'])))
	#top_voxels_T.append(fast_top_voxels(ds_all[i]['data'], ds_all[0]['data'], len(ds_all[0]['data'])))
...
for topz in range(100, 1000, 100):
	...
	#for i in range(15,16,1):
		...
		#for j in range (30,40,10):
			...
			for k in range(1, len(ds_all)):
				Ss.append(copy.deepcopy(ds_all[k]))
				Ts.append(copy.deepcopy(ds_all[0]))


			for k in range(0, len(Ss)):
				Ss[k]['data'] = Ss[k]['data'][top_voxels_S[k][-topz:]]
			...
			for k in range(0, len(Ss)):
				...
				pca.fit(Ts[k]['data'].transpose())
				Ts[k]['data'] = pca.transform(Ts[k]['data'].transpose()).transpose()
			...
			for e in range(0, len(sData)):
				...
				D_train, D_test, L_train, L_test = cross_validation.train_test_split(tData[e], labels, test_size = (float(j)/100), random_state=0)
