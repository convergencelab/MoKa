...
csvout = csv.writer(open('statz3_2.csv', 'w'))
...
for i in range(0, len(sc)):
	tcc[i]['data'] = tc[i]['data'][topV_t[i][:]]
	scc[i]['data'] = sc[i]['data'][topV_s[i][:]]
...
for PC in range(2, 40, 1):
	...
	print "\n"
	csvout.writerow([PC, numpy.mean(score1), numpy.mean(score2)])
