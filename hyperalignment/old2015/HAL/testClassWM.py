...
def get_labels_no_cue_cut_edges(TR, EV):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 2), int(numpy.ceil(ep) - 15)):	#was 5
			labels.append(ev)
...
def get_data_for_class_no_cue_cut_edges(TR, EV, inData):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 2), int(numpy.ceil(ep) - 15)):	#was5
			data.append(inData[j])
...
s = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii.gz',datapath+'primary visual_pFgA_z_FDR_0.01.nii.gz',1)
t = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii.gz',datapath+'primary visual_pFgA_z_FDR_0.01.nii.gz',1)
...
#top_s = fast_top_voxels(s['data'],t['data'],100)
#top_t = fast_top_voxels(t['data'],s['data'],100)
...
myGuy2 = KNeighborsClassifier(2)

D_train, D_test, L_train, L_test = cross_validation.train_test_split(tData, labels, test_size = 0.2, random_state=0)
