...
def get_labels_no_cue_cut_edges(TR, EV):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 4), int(numpy.ceil(ep) - 25)):	#was 5
			labels.append(ev)
...
def get_data_for_class_no_cue_cut_edges(TR, EV, inData):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 4), int(numpy.ceil(ep) - 25)):	#was5
			data.append(inData[j])
...
inputs = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'temporal_pFgA_z_FDR_0.01.nii.gz',5.5)
#inputs = xi.python_vts(datapath+'WorkingMemory/101006/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'temporal_pFgA_z_FDR_0.01.nii.gz',5.5)
inputt = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'temporal_pFgA_z_FDR_0.01.nii.gz',5.5)


#inputs = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'visual_pFgA_z_FDR_0.01.nii.gz',0.5)
#inputt = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'visual_pFgA_z_FDR_0.01.nii.gz',0.5)

#inputs = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'ventral visual_pFgA_z_FDR_0.01.nii.gz',0.1)
#inputt = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'ventral visual_pFgA_z_FDR_0.01.nii.gz',0.1)

#inputs = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'anterior temporal_pFgA_z_FDR_0.01.nii.gz',0.1)
#inputt = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'anterior temporal_pFgA_z_FDR_0.01.nii.gz',0.1)

#inputs = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'primary visual_pFgA_z_FDR_0.01.nii.gz',0.5)
#inputt = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii',datapath+'primary visual_pFgA_z_FDR_0.01.nii.gz',0.5)

print inputs['data'].shape

print 'Top Voxels'

top_s = fast_top_voxels(inputs['data'],inputt['data'],len(inputs['data']))
top_t = fast_top_voxels(inputt['data'],inputs['data'],len(inputt['data']))
...
#10,10
#11,10
#to get .6875, try T, 5.5, 200, 11, 30, 1NN (oddly small eh)
#for topz in range(100, 2000, 100):
for topz in range(200, 300, 100):
	csvout.writerow([' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',topz])
	#for i in range(8,50,1):
	for i in range(11,12,1):
		bleh = []
		print '-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-'	
		#for j in range (30,60,10):
		for j in range (30,40,10):
			...
			#########################
			#CLASSIFICATION STUFF HERE (This is where I would change how I do MVPA stuff --- BETA, VOXELS, ETC.)good class

			rawLabels = load_EVs_data(datapath+'WorkingMemoryEVs.csv')
			...
			#s['data'] = numpy.array(sData).transpose() ###
			#t['data'] = numpy.array(tData).transpose()	###		
			...
			score = myGuy2.score(D_test, L_test)
			...
			cmap=plt.cm.Blues
			plt.imshow(cm_normalized_KNN, interpolation='nearest', cmap=cmap)
			
			for asd, cas in enumerate(cm_normalized_KNN):
