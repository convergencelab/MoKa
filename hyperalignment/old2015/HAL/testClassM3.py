...
def get_labels_no_cue_cut_edges(TR, EV):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 3), int(numpy.ceil(ep) - 5)):	#was 5
			labels.append(ev)
...
def get_data_for_class_no_cue_cut_edges(TR, EV, inData):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 3), int(numpy.ceil(ep) - 5)):	#was5
			data.append(inData[j])
...
def get_labels_no_cue_no_t_cut_edges(TR, EV):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 3), int(numpy.ceil(ep) - 5)):
			labels.append(ev)
...
inputs = xi.python_vts(datapath+'Motor/100307/MNINonLinear/Results/tfMRI_MOTOR_LR/tfMRI_MOTOR_LR.nii',datapath+'primary motor_pFgA_z_FDR_0.01.nii.gz',0.5) #was 9
inputt = xi.python_vts(datapath+'Motor/100408/MNINonLinear/Results/tfMRI_MOTOR_LR/tfMRI_MOTOR_LR.nii',datapath+'primary motor_pFgA_z_FDR_0.01.nii.gz',0.5)
...
#10,10
#11,10
for topz in range(200, len(inputs['data']), 100):
#for topz in range(1100, 1101, 100):
	csvout.writerow([' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',topz])
	for i in range(8,80,1):
	#for i in range(8,9,1):
		...
		#for j in range (20,21,10):
			...
			cmap=plt.cm.Blues
			plt.imshow(numpy.array(cm_normalized_KNN), interpolation='nearest', cmap=cmap)
			plt.title('1NN')
			plt.colorbar()
			tick_marks = numpy.arange(len(numpy.unique(labels)))
			plt.xticks(tick_marks, numpy.unique(labels), rotation=45)
