...
def get_labels_no_cue_cut_edges(TR, EV):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 4), int(numpy.ceil(ep) - 22)):	#was 5
			labels.append(ev)
...
def get_data_for_class_no_cue_cut_edges(TR, EV, inData):
	...
	for i in EV:
		...
		ep = sp + float(i[1])/TR	
		ev = i[2]
		for j in range(int(numpy.floor(sp) + 4), int(numpy.ceil(ep) - 22)):	#was5
			data.append(inData[j])
...
#s = xi.python_vts(datapath+'WorkingMemory/100307/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii.gz',datapath+'primary visual_pFgA_z_FDR_0.01.nii.gz',3)
#t = xi.python_vts(datapath+'WorkingMemory/100408/MNINonLinear/Results/tfMRI_WM_LR/tfMRI_WM_LR.nii.gz',datapath+'primary visual_pFgA_z_FDR_0.01.nii.gz',3)
...
top_s = fast_top_voxels(s['data'],t['data'],1250)
top_t = fast_top_voxels(t['data'],s['data'],1250)
...
D_train, D_test, L_train, L_test = cross_validation.train_test_split(tData, labels, test_size = 0.35, random_state=0)
...
T,b = build_xform(new_s,t)
new_s = apply_xform(new_s,T,b)

print numpy.array_equal(t['data'], new_s['data'])
